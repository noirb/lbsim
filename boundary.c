				...
				if (flag == FLUID)
				{
					...
					if (flag == MOVING_WALL)
					{
						...
						for (l = 0; l<19; l++)
						{
							...
							ind = INDEXOF(xlength, i + LATTICEVELOCITIES[l][0], j+LATTICEVELOCITIES[l][1], k+LATTICEVELOCITIES[l][2], l);
	...
	for(i = 1; i <= xlength; i++)
		for (j=1; j<= xlength; j++)
		{
			...
			if (flag == MOVING_WALL)
			{
				...
				for(l = 0; l<N; l++)
				{
				ind = INDEXOF(xlength, i + LATTICEVELOCITIES[oyz[l]][0], j+LATTICEVELOCITIES[oyz[l]][1], LATTICEVELOCITIES[oyz[l]][2], oyz[l]);
				computeDensity(collideField+ind, &den);
			...
			if (flag == MOVING_WALL)
			{
				for(l = 0; l<N; l++)
				{
					ind = INDEXOF(xlength, i + LATTICEVELOCITIES[xyn[l]][0], j+LATTICEVELOCITIES[xyn[l]][1], LATTICEVELOCITIES[xyn[l]][2], xyn[l]);
					computeDensity(collideField+ind, &den);
	...
	for(j = 1; j <= xlength; j++)
			for (k=1; k<= xlength; k++)
			{
				...
				if (flag == MOVING_WALL) //update directions 1, 5, 8, 11, 15
					...
					for(l = 0; l<N; l++)
					{
					ind = INDEXOF(xlength, i + LATTICEVELOCITIES[nyz[l]][0], j+LATTICEVELOCITIES[nyz[l]][1], LATTICEVELOCITIES[nyz[l]][2], nyz[l]);
					computeDensity(collideField+ind, &den);
	...
	//X0Z and XNZ
		for(i = 1; i <= xlength; i++)
				for (k=1; k<= xlength; k++)
				{
					...
					if (flag == MOVING_WALL) //update directions 4, 11, 12, 13, 18
					{
						...
						for(l = 0; l<N; l++)
						{
							ind = INDEXOF(xlength, i + LATTICEVELOCITIES[xoz[l]][0], j+LATTICEVELOCITIES[xoz[l]][1], LATTICEVELOCITIES[xoz[l]][2], xoz[l]);
							computeDensity(collideField+ind, &den);
					...
					if (flag == MOVING_WALL) //update directions 0, 5, 7, 6, 14
					{
						...
						for(l = 0; l<N; l++)
						{
							ind = INDEXOF(xlength, i + LATTICEVELOCITIES[xnz[l]][0], j+LATTICEVELOCITIES[xnz[l]][1], LATTICEVELOCITIES[xnz[l]][2], xnz[l]);
							computeDensity(collideField+ind, &den);
		...
		for(i = 1; i<=xlength; i++)
		{
			...
			if (flag == FLUID)
				...
				if (flag == MOVING_WALL)
				{
					ind = INDEXOF(xlength, i + LATTICEVELOCITIES[18][0], LATTICEVELOCITIES[18][1], LATTICEVELOCITIES[18][2], 18);
					computeDensity(collideField+ind, &den);
				...
				if (flag == FLUID)
					...
					if (flag == MOVING_WALL)
						{
						ind = INDEXOF(xlength, i + LATTICEVELOCITIES[4][0], LATTICEVELOCITIES[4][1], LATTICEVELOCITIES[4][2], 4);
						computeDensity(collideField+ind, &den);
				...
				if (flag == FLUID)
					...
					if (flag == MOVING_WALL)
					{
						ind = INDEXOF(xlength, i + LATTICEVELOCITIES[14][0], xlength+1+LATTICEVELOCITIES[14][1], LATTICEVELOCITIES[14][2], 14);
						computeDensity(collideField+ind, &den);
		...
		for(j = 1; j<=xlength; j++)
		{
			...
			if (flag == FLUID)
				...
				if (flag == MOVING_WALL)
				{
					ind = INDEXOF(xlength, LATTICEVELOCITIES[17][0], j+LATTICEVELOCITIES[17][1], LATTICEVELOCITIES[17][2], 17);
					computeDensity(collideField+ind, &den);
				...
				if (flag == FLUID)
					...
					if (flag == MOVING_WALL)
						{
						ind = INDEXOF(xlength, xlength+1 + LATTICEVELOCITIES[15][0], j+LATTICEVELOCITIES[15][1], LATTICEVELOCITIES[15][2], 15);
						computeDensity(collideField+ind, &den);
				...
				if (flag == FLUID)
					...
					if (flag == MOVING_WALL)
					{
						ind = INDEXOF(xlength, LATTICEVELOCITIES[3][0], j+LATTICEVELOCITIES[3][1], xlength+1+LATTICEVELOCITIES[3][2], 3);
						computeDensity(collideField+ind, &den);
				...
				if (flag == FLUID)
					...
					if (flag == MOVING_WALL)
					{
						ind = INDEXOF(xlength, xlength+1 + LATTICEVELOCITIES[1][0], j+LATTICEVELOCITIES[1][1], xlength+1+LATTICEVELOCITIES[1][2], 1);
						computeDensity(collideField+ind, &den);
		...
		for(k = 1; k<=xlength; k++)
		{
			...
			if (flag == FLUID)
				...
				if (flag == MOVING_WALL)
				{
					ind = INDEXOF(xlength, LATTICEVELOCITIES[13][0], LATTICEVELOCITIES[13][1], k+LATTICEVELOCITIES[13][2], 13);
					computeDensity(collideField+ind, &den);
				...
				if (flag == FLUID)
					...
					if (flag == MOVING_WALL)
						{
						ind = INDEXOF(xlength, xlength+1 + LATTICEVELOCITIES[11][0], LATTICEVELOCITIES[11][1], k+LATTICEVELOCITIES[11][2], 11);
						computeDensity(collideField+ind, &den);
				...
				if (flag == FLUID)
					...
					if (flag == MOVING_WALL)
					{
						ind = INDEXOF(xlength, LATTICEVELOCITIES[7][0], xlength+1+LATTICEVELOCITIES[7][1], k+LATTICEVELOCITIES[7][2], 7);
						computeDensity(collideField+ind, &den);
				...
				if (flag == FLUID)
					...
					if (flag == MOVING_WALL)
					{
						ind = INDEXOF(xlength, xlength+1+LATTICEVELOCITIES[5][0], xlength+1+LATTICEVELOCITIES[5][1], k+LATTICEVELOCITIES[5][2], 5);
						computeDensity(collideField+ind, &den);
