...
#ifndef _WIN32 // regex matching is handled differently on Windows than Linux, so this is implemented twice
...
int initialiseFields(double *collideField, double *streamField, flag_data *flagField, int xlength, int ylength, int zlength, char* cellDataFile)
{
    ...
    char  temp[80];
    int   nline = 0;
    std::regex regex_single("\\((\\*|[nN]|[0-9]+) +(\\*|[nN]|[0-9]+) +(\\*|[nN]|[0-9]+)\\) +([A-Z]+_*[A-Z]*) *(\\((-?[0-9]+\\.[0-9]+|-?[0-9]+) *(-?[0-9]+\\.[0-9]+|-?[0-9]+)? *(-?[0-9]+\\.[0-9]+|-?[0-9]+)?\\))?");
    vary_flags wildCardFlags = VARY_NONE;
...
#else
    errno_t err = fopen_s(&cellData, cellDataFile, "r");
    if ( err != 0)
    {
        fprintf(stderr, "ERROR: Could not open cell data file: %s\n Error Code: %d\n", cellDataFile, err);
        exit(1);
    ...
    while (fgets(line, 80, cellData) != NULL)
    {
        ...
        // check for cell data on current line & parse it if found
        std::cmatch matches;
        if ( std::regex_search(line, matches, regex_single) )
        {
            for (int i = 0; i < matches.size(); i++)
            {
                __int64 len = matches[i].length();
                strncpy_s(temp, 80, matches[i].str().c_str(), len);
                temp[len] = 0; // null-terminate resulting string
                ...
                case 5: // contains all parameter groups
                    ...
                    break;
                case 6: // first parameter
                    sscanf_s(temp, "%lf", cellParms);
                    break;
                case 7: // second parameter
                    sscanf_s(temp, "%lf", cellParms + 1);
                    break;
                case 8: // third parameter
                    sscanf_s(temp, "%lf", cellParms + 2);
                    break;
                default:
                    continue;
                }
